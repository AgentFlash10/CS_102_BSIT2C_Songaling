...
write_csv(filtered_amazonreviews, "AmazonReviews_filtered/cleaned.csv")
```
...
filtered_papers <- arxiv_paper %>%
  mutate(date = datex) %>%
  mutate(subject = gsub("\\s\\(.*\\)", "", subject),
         across(where(is.character), tolower)) %>%
         select(-meta, -...1)

write.csv(cleaned_droga, "Sports_arxivpapers_filtered/cleaned.csv")
```