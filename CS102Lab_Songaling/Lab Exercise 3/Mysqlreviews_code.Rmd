...
```{r}
movierev <- read.csv("movie_reviews.csv")
```

```{r}
#getting all data using SELECT * FROM dbaseName
review_data <- dbGetQuery(connection, "SELECT * FROM movie_reviews.movie_revs")
...
```{r}
#getting all data using SELECT * FROM dbaseName
SqlQuery <- "SELECT movie_title,rating FROM movie_reviews.movie_revs"
...
```{r}
#3 
query <- "INSERT INTO movie_revs (movie_title, reviewer_name, date, rating, review_content, review_title) VALUES ( 'Avatar: The Way of Water','dommercaldi','29 January 2023','4/10','Pros:
